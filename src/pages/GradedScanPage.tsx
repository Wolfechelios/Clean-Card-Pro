...
export default function GradedScanPage() {
  ...
  const processImage = useCallback(async (imageUrl: string) => {
    setScanning(true);
    try {
      const { data, error } = await disabledSupabaseFunctionInvoke("analyze-graded-card", {
        body: { imageUrl },
      });
  ...
  const verifyCard = async (data: GradedCardData) => {
    ...
    setVerifying(true);
    try {
      const { data: verificationResult, error } = await disabledSupabaseFunctionInvoke("verify-graded-card", {
        body: {
          certNumber: data.certNumber,
          gradingCompany: data.gradingCompany,
        },
  ...
  const capturePhoto = async () => {
    ...
    try {
      ...
      const { fileToDataUrl } = await import("@/lib/local/fileToDataUrl");
      const publicUrl = await fileToDataUrl(blob);
      await processImage(publicUrl);
    } catch (err) {
