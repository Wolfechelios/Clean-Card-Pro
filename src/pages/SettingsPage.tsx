...
import { LogOut, Trash2, User, Lock, ImageOff, Clock, RefreshCw, Database, ScanLine, ImageIcon, Wand2, Cpu, Key, Monitor, Search, Smartphone, ExternalLink } from "lucide-react";
            ...
            <div className="grid gap-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-mono text-muted-foreground">
                  {Number(scannerSettings.autoTimerIntervalSeconds).toFixed(1)}s
                </span>
              </div>
              <input
                type="range"
              ...
              <div className="flex flex-wrap gap-2">
                {[1, 1.1, 1.25, 1.5, 2, 5].map((v) => (
                  <button
                    key={v}
                    type="button"
                    onClick={() => updateScannerSettings({ autoTimerIntervalSeconds: v as any })}
                    className={`px-2 py-1 rounded-md border text-xs ${
                      Number(scannerSettings.autoTimerIntervalSeconds) === v
