...
export type ProcessorState = {
  ...
  debugEvents: RapidDebugEvent[];
  lastDebugEvent: RapidDebugEvent | null;
};
...
type ProcessorStore = ProcessorState & {
  ...
  _pushDebugEvent: (event: RapidDebugEvent) => void;
  clearDebug: () => void;
...
export const useQueueProcessor = create<ProcessorStore>((set, get) => ({
  ...
  start: () => {
    if (get().isRunning) {
      emitDebug("processor", "Start ignored — processor already running");
      return;
    }
    if (readAnomalyPauseFlag()) {
      set({ isPaused: true, isPausedByAnomaly: true });
      emitDebug("paused", "Start blocked — anomaly pause is still active");
      return;
    }
    queueAnomalyDetector.resetSession();
    set({ isRunning: true, isPaused: false, isPausedByAnomaly: false });
    emitDebug("processor", "Processor started");
    startWorkers();
  },

  stop: () => {
    emitDebug("processor", "Processor stopped");
  ...
  resume: () => {
    emitDebug("processor", "Processor resume requested");
  ...
  _pushDebugEvent: (event) => set((state) => ({
    lastDebugEvent: event,
    debugEvents: [event, ...state.debugEvents].slice(0, 80),
  })),
  clearDebug: () => set({ debugEvents: [], lastDebugEvent: null }),
  _incrementProcessed: () => set((s) => ({ processedCount: s.processedCount + 1 })),
  _incrementError: () => set((s) => ({ errorCount: s.errorCount + 1 })),
}));

function emitDebug(stage: string, message: string, itemId?: string | null, details?: Record<string, unknown>) {
  try {
    useQueueProcessor.getState()._pushDebugEvent({
      at: Date.now(),
      stage,
      message,
      itemId: itemId ?? null,
    ...
    // debug must never break scanning
  }
}
  ...
  if (!scalingInterval) {
    scalingInterval = setInterval(async () => {
      ...
      while (workersActive < targetWorkers && store.isRunning) {
        ...
        emitDebug("worker", `Starting worker ${newWorkerId}`, null, { workersActive, queueSize });
  ...
  while (store().isRunning) {
    ...
    if (!next) {
      ...
      if (consecutiveEmpty >= 3) {
        ...
        if (queuedCount === 0) {
          emitDebug("queue", "No queued cards left — processor idle");
...
async function processJob(item: QueueItem): Promise<void> {
  ...
  emitDebug("processing", "Picked queued photo", item.id);
  ...
  emitDebug("image", "Preparing captured image", item.id, { mime: item.mime, size: item.blob.size });
  ...
  emitDebug("identify", "Starting rapid card identify", item.id, { gameTypeHint });
  ...
  if (!(identifyInitial as any)?.error && (identifyInitial as any)?.success) {
    ...
    emitDebug("identify", `Identified: ${identify?.card_name ?? "Unknown"}`, item.id, { source: (identifyInitial as any).source, confidence: initialConfidence, set: identify?.card_set, number: identify?.card_number });
    ...
    emitDebug("identify", "Initial identify failed — trying OCR fallback", item.id, { error: String(err?.message ?? err ?? "unknown") });
  }
  ...
  if (shouldUseOcrFallback) {
    emitDebug("ocr", "Starting OCR fallback", item.id);
    ...
    if (ocrResult && (ocrResult as any).data && !(ocrResult as any).error) {
      ...
      emitDebug("ocr", "OCR returned text", item.id, { confidence: ocrConfidence, setCode: ocrSetCode, cardNumber: ocrCardNumber, sample: ocrText?.substring(0, 80) });
    ...
    if (ocrText) {
      ...
      emitDebug("identify", `OCR retry identified: ${identify?.card_name ?? "Unknown"}`, item.id, { source: retryResult.source, set: identify?.card_set, number: identify?.card_number, confidence: identify?.confidence });
  ...
  if (cardName === "Unknown Card" || confidence < MIN_CONFIDENCE) {
    ...
    emitDebug("discard", `Discarded unreadable card`, item.id, { confidence, cardName });
  ...
  emitDebug("price", "Fetching price and uploading image", item.id, { cardName, cardSet, cardNumber });
  ...
  emitDebug("done", `Completed: ${cardName}`, item.id, { rawPrice, psa10Price, isInLibrary: processedCard.isInLibrary });
  ...
  await idbDelete(item.id);
  emitDebug("queue", "Removed completed item from queue", item.id);
}
...
export async function checkAndResumeQueue(): Promise<void> {
  ...
  if (queuedCount > 0) {
    ...
    emitDebug("processor", `Auto-resuming ${queuedCount} queued item(s)`);
    state.start();
  }
}
