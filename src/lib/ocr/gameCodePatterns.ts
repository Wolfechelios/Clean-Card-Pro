...
export function extractPrintedCode(rawText: string): DetectedCode {
  ...
  if (pkmFrac && (isPokemon || !isMtg)) {
    const num = fixNumberSection(pkmFrac[1]);
    const total = fixNumberSection(pkmFrac[2]);
    candidates.push({ game: "pokemon", setCode: null, cardNumber: `${num}/${total}`, fullCode: `${num}/${total}`, edition: null, rawMatch: pkmFrac[0], confidence: 0.6 + (isPokemon ? 0.25 : 0) });
  } else if (!pkmFrac) {
    const promo = upper.match(POKEMON_PROMO_RE);
    if (promo) {
      const setCode = promo[1].toUpperCase();
      const num = fixNumberSection(promo[2]);
      candidates.push({ game: "pokemon", setCode, cardNumber: num, fullCode: `${setCode}${num}`, edition: null, rawMatch: promo[0], confidence: 0.6 + (isPokemon ? 0.25 : 0) });
    }
  }
