...
type NormalizedRow = YugiohSetCodeIndexMatch & {
  searchName: string;
  searchSet: string;
  keys: string[];
};
...
function asArray(json: any): AnyRow[] {
  ...
  if (json && typeof json === "object") {
    return Object.entries(json).map(([key, value]) => {
      if (value && typeof value === "object") return { lookupKey: key, ...(value as AnyRow) };
      return { lookupKey: key, value };
    });
  }
  return [];
}
...
export function normalizeYugiohPrintedCode(value: string | null | undefined): string | null {
  ...
  if (withDash) {
    const prefix = withDash[1];
    const country = withDash[2] ?? "";
    const number = withDash[3];
...
function normalizeRow(row: AnyRow): NormalizedRow | null {
  ...
  return {
    ...
    currentPricePsa9,
    currentPricePsa10,
    suggestedPrice: suggestedPrice ?? currentPriceRaw,
    priceChartingUrl,
    confidence: 0,
    searchName: normalizeSearch(cardName),
    searchSet: normalizeSearch(cardSet),
    keys: buildKeys(row, cardNumber, setCode),
  };
}
...
async function loadRows(): Promise<NormalizedRow[]> {
  ...
  if (!indexPromise) {
    indexPromise = fetch(INDEX_URL, { cache: "force-cache" })
      ...
      .catch((error) => {
        console.warn("[yugiohSetCodeIndex] Failed to load local index:", error);
        return [];
      });
  }
  return indexPromise;
...
function finish(row: NormalizedRow, confidence: number): YugiohSetCodeIndexMatch {
  return {
    ...
    setCode: row.setCode,
    rarity: row.rarity,
    currentPriceRaw: row.currentPriceRaw,
    currentPricePsa9: row.currentPricePsa9,
    currentPricePsa10: row.currentPricePsa10,
...
export async function findYugiohSetCodeIndexMatch(args: {
  ...
  const addCode = (value?: string | null) => {
    const normalized = normalizeYugiohPrintedCode(value);
  ...
  const rawMatches = raw.match(/[A-Z0-9]{2,12}[\s\-]?(?:[A-Z]{2})?[\s\-]?\d{2,5}[A-Z]?/gi) ?? [];
  rawMatches.forEach(addCode);

  if (args.setCode && args.cardNumber) {
    addCode(`${args.setCode}-${args.cardNumber}`);
    addCode(`${args.setCode}${args.cardNumber}`);
  }
