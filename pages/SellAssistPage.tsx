...
import { ArrowLeft, Calculator, Copy, Loader2, Sparkles } from "lucide-react";
import type { CardData } from "@/components/cards/CardDetailModal";
import { ResaleProfitCalculator } from "@/components/pricing/ResaleProfitCalculator";
...
export default function SellAssistPage() {
  ...
  const runAssist = async () => {
    ...
    try {
      ...
      const { data, error } = await disabledSupabaseFunctionInvoke("sell-assist", {
        body: payload,
      });
  ...
  const [activeTab, setActiveTab] = useState<"ai-assist" | "calculator">("ai-assist");
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Listing strategy, profit calculator & ready-to-post copy.
            </p>
          </div>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as typeof activeTab)} className="space-y-4">
        ...
        <TabsContent value="calculator" className="space-y-6">
          <ResaleProfitCalculator
            initialSalePrice={card?.current_price_raw ?? 0}
            initialCost={0}
          />
        </TabsContent>
      </Tabs>
