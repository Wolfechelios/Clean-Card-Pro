"""Jetson Orin Inference Server — TensorRT-accelerated card scanning.

Same API contract as mac-inference-server so the client connects seamlessly.
Endpoints: /health, /identify, /ocr, /ws/stream
"""
...
app = FastAPI(title="Jetson Orin Vision Accelerator", version=APP_VERSION)
...
async def _start_mdns():
    try:
        from server.discovery import start_mdns_announcement
        start_mdns_announcement(port=8000)
    except Exception as e:
        print(f"[mdns] Could not start mDNS announcement: {e}")


@app.on_event("shutdown")
async def _stop_mdns():
    try:
        from server.discovery import stop_mdns_announcement
...
def _gpu_info() -> Dict[str, Any]:
    """Return Jetson GPU stats if available."""
    info: Dict[str, Any] = {}
    try:
        import subprocess
        ...
        info["tegrastats_raw"] = out.strip()
    except Exception:
        pass

    try:
        with open("/sys/devices/gpu.0/load", "r") as f:
            info["gpu_load_pct"] = int(f.read().strip()) / 10.0
    except Exception:
        pass

    try:
        with open("/sys/class/thermal/thermal_zone0/temp", "r") as f:
            info["gpu_temp_c"] = int(f.read().strip()) / 1000.0
    except Exception:
        pass
