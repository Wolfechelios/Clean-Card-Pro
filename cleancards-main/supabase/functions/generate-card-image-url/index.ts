...
async function searchScryfall(cardName: string, cardSet?: string): Promise<string | null> {
  try {
    const response = await fetchWithTimeout(
      `${SCRYFALL_API}?fuzzy=${encodeURIComponent(cardName)}`,
      { method: 'GET' },
      8000,
    );
...
async function searchPokemonTCG(cardName: string, cardSet?: string): Promise<string | null> {
  try {
    ...
    const response = await fetchWithTimeout(
      `${POKEMON_TCG_API}?q=${encodeURIComponent(query)}&pageSize=1`,
      { method: 'GET' },
      8000,
    );
...
async function searchWithPerplexity(cardName: string, cardSet: string | null, gameType: string | null): Promise<string | null> {
  ...
  try {
    const searchQuery = [cardName, cardSet, gameType, 'trading card image']
      .filter(Boolean)
      .join(' ');
...
Never fabricate URLs - only return URLs you find in search results.`,
            ...
            {
              role: 'user',
              content: `Find a direct image URL for this trading card: ${cardName}${
                cardSet ? ` from set "${cardSet}"` : ''
              }${gameType ? ` (${gameType})` : ''}`,
            },
          ],
    ...
    if (!response.ok) {
      // Important: do NOT read large HTML error bodies (can cause connection resets)
    ...
    try {
      ...
      console.error('Perplexity JSON parse failed:', error);
      return null;
    }
