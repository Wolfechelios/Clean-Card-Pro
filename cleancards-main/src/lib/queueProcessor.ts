...
import { confidencePct, normalizeConfidence01 } from "@/lib/confidence";
import {
  idbGetNextQueued,
  idbUpdateMeta,
  idbDelete,
...
async function processJob(item: QueueItem): Promise<void> {
  ...
  if (!(identifyInitial as any)?.error && (identifyInitial as any)?.success) {
    ...
    const initialConfidence = normalizeConfidence01(identify?.confidence);
  ...
  const confidence: number = normalizeConfidence01(identify?.confidence);
  ...
  const confPct = confidencePct(confidence);
